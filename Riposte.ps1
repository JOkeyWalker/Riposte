...
function Invoke-GlobalHunt {
    ...
    # Capture hunt start time — events at/after this moment are from this script's own execution
    ...
    foreach ($target in $eventLogTargets) {
        ...
        try {
            ...
            foreach ($evt in $events) {
                ...
                # Skip SentinelOne remote shell operator events
                if ($evtUser -match "(?i)SentinelRSHUser|SentinelOne") { continue }

                # NOTE: Not filtering by SYSTEM/NETWORK SERVICE account here - malicious activity
                # (rogue services, RMM tools) commonly runs under these accounts. Self-noise from
                # this scan is instead excluded by timestamp below.

                # Skip events generated by this script's own execution (same-session PS logs)
...
function Get-PSHistory {
    ...
    if ($events) {
        foreach ($event in $events) {
            ...
            # NOTE: We do NOT filter by account (SYSTEM/systemprofile/NETWORK SERVICE) here.
            # Legitimate attacker activity (e.g. a rogue RMM tool executing PowerShell) commonly
            # runs under these same SYSTEM-level accounts, so filtering by account would create
            # a blind spot. Riposte's own self-noise is instead filtered below by content pattern
            # (its own function definitions, banner text, etc.) which is precise rather than broad.
...
function Invoke-EventLogSearch {
    param(
        [hashtable]$parsedTime,
        [array]$keywords,
        [string]$regexPattern
    )
    $huntStartTime = Get-Date
...
function Get-RMMHunt {
    ...
    $rmmTools = @(
        ...
        @{ Name = "eBLVD";               Exes = @("eblvd.exe","eblvdhost.exe","eblvd-host.exe","eblvdservice.exe");            Services = @("eBLVD","eBLVD Host","eBLVDService");           RegKeys = @("eBLVD");                      Paths = @("$env:ProgramFiles\eBLVD","${env:ProgramFiles(x86)}\eBLVD","$env:APPDATA\eBLVD") },
    ...
    foreach ($tool in $rmmTools) {
        ...
        foreach ($det in $detections) {
            ...
            if ($tool.Name -eq "ScreenConnect") {
                $configDir = $null
                # Try all available path hints to locate THIS SPECIFIC detection's install folder.
                # Do NOT fall back to "any ScreenConnect folder found on disk" - when multiple
                # instances exist (e.g. a legitimate one + a rogue one), that fallback attaches
                # the wrong instance's relay to every detection that lacks a clear path hint.
                ...
                Get-ChildItem $root -Recurse -Depth 1 -Filter $dbName[$fam] -File -ErrorAction SilentlyContinue | ForEach-Object {
                    ...
                    try {
                        ...
                        foreach ($r in $hits) {
                            ...
                            $coreKey = "$([int]$r[5])"
                            $core = if ($script:BH_CORE.ContainsKey($coreKey)) { $script:BH_CORE[$coreKey] } else { "Type$coreKey" }
                            ...
                            try {
                                ...
                                foreach ($d in [BH.Sqlite3]::Query($copy, $dSql)) {
                                    $rows.Add([PSCustomObject]@{
                                        ...
                                        Name       = [string]$d[6]
                                        URL        = [string]$d[1]
                                        Transition = if ($script:BH_DLSTATE.ContainsKey("$([int]$d[4])")) { $script:BH_DLSTATE["$([int]$d[4])"] } else { "Unknown" }
                                        Duration   = $null
                                        Visits     = $null
                                        Bytes      = [int64]$d[2]
                                    })
